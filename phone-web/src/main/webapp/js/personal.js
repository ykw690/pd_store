...
$(".save_recipient").click(
		function() {
			...
			if (receiverName && receiverState && receiverCity
					&& receiverDistrict && receiverAddress && receiverMobile) {
				$("form").submit();
			} else {
				alert("请将必填信息填写完整");
			}
...
function setDefault(e) {
	var addressId = $(e).attr("data_addressId");
	var url = "/address/setDefault.html";
	var param = {
		"addId" : addressId
	};
	$.ajax({
		...
		"success" : function(response) {
			...
			if (response.status == 200) {
				...
				if ($(parent).siblings().hasClass("aim_active")) {
					...
					$(parent).siblings().children(".dzmc_active").removeClass(
							"dzmc_active").addClass("dzmc_normal");
					$(parent).siblings().children(".swmr_normal").html("设为默认")
				}
				$(parent).addClass("aim_active");
				$(parent).children(".dzmc_normal").removeClass("dzmc_normal")
						.addClass("dzmc_active");
				$(e).html("");
			}
		},
		"error" : function(e) {
...
$(function() {
	$(".aco_delete").click(function() {
		if (confirm("确定删除吗？")) {
			...
			// },"json");
			var id = this.id;
			var url = "/address/delete.html";
			// jquery框架把js对象转成addId=id&username=a
			var param = {
				"addId" : id
			}// 发给服务器的数据
			$.ajax({
